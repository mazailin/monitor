...
for(var i=0;i<scripts.length;i++){
	if(/.*dwz\/js\/Dialog\.js$/g.test(scripts[i].getAttribute("src"))){
		var jsPath = scripts[i].getAttribute("src").replace(/dwz\/js\/Dialog\.js$/g,'');
...
var Server = {};
Server.ContextPath = CONTEXTPATH+'styles/';
...
//选择小时
Calendar.SelHours = function (value) { this.Time.Hours = value; Calendar.setDate();}
//选择分钟
Calendar.SelMinutes = function (value) { this.Time.Minutes = value; Calendar.setDate();}
//选择秒
Calendar.SelSeconds = function (value) { this.Time.Seconds = value;Calendar.setDate(); }
Calendar.getDateString = function(cell){
	var win = $o("_Calendar_Frame").contentWindow;
	var Control = win.Control;
	var format = Control.$A("format");
	var ztype = Control.$A("ztype");
	if(!format){
		format = "yyyy-MM-dd";
	}
	...
	if(cell.Day){
		...
		Calendar.Time.Year=year; 
		Calendar.Time.Month=month; 
		Calendar.Time.Day=day;
		
		var d; 
	if (ztype == "datetime")  
	d = new Date(Calendar.Time.Year, Calendar.Time.Month, Calendar.Time.Day, Calendar.Time.Hours, Calendar.Time.Minutes, Calendar.Time.Seconds); 
	else d = new Date(Calendar.Time.Year, Calendar.Time.Month, Calendar.Time.Day); 
...
Calendar.previousYear = function(){
	var win = $o("_Calendar_Frame").contentWindow;
	var year = win.$o("_Calendar_Year").Year;
	Calendar.Time.Year =--year;
	Calendar.setDate();
	Calendar.adjustSize();
}
...
Calendar.setDate = function(date){
	...
	win.$o("_Calendar_Tip").innerHTML =d.formatDate(format);
	d2 = new Date();
...
Calendar.show = function(ctrl,date){
	...
	date = date+"";
	var type=(ctrl.$A("ztype"));
	var format = ctrl.$A("format");
		if(!format){
			format = "yyyy-MM-dd";
		}
	var d;
	...
	if(!date){
		 ...
		 d= date.parseDate(format);		
	}
	  Calendar.Time.Year =isGecko?d.getYear()+1900:d.getYear();
      Calendar.Time.Month=d.getMonth();
      Calendar.Time.Day = d.getDate();
	  if(type  == "datetime"){		 
	        Calendar.Time.Hours = d.getHours();
            Calendar.Time.Minutes =d.getMinutes();
            Calendar.Time.Seconds = d.getSeconds();
		}
	...
	if(!pw.$o("_Calendar")){
		...
		if(type == "datetime"){		
		arr.push("    <TR id=_Calendar_timeRow><TD class=day colSpan=7>");
		arr.push( "<div style=\"margin:0;padding:2px 3px 0;\">时间<select onchange=\"TopWindow.Calendar.SelHours(this.value);\">");
    for (var i = 0; i <= 23; i++) {arr.push(  "<option value=\"" + i + "\" " + (this.Time.Hours == i ? "selected" : "") + ">" + (i < 10 ? "0" + i : i) + "</option>"); }
    arr.push(  "</select> : <select onchange=\"TopWindow.Calendar.SelMinutes(this.value);\">");
    for (var i = 0; i <= 59; i++) { arr.push(  "<option value=\"" + i + "\" " + (this.Time.Minutes == i ? "selected" : "") + ">" + (i < 10 ? "0" + i : i) + "</option>"); }
    arr.push(  "</select> : <select onchange=\"TopWindow.Calendar.SelSeconds(this.value);\">");
    for (var i = 0; i <= 59; i++) { arr.push(  "<option value=\"" + i + "\" " + (this.Time.Seconds == i ? "selected" : "") + ">" + (i < 10 ? "0" + i : i) + "</option>"); }
   arr.push(  "</select></div>");
	arr.push("    </TD></TR>");
	}
		arr.push("  </TBODY>");
